...
From there, James and John and I shared a series of emails (subject: treatment dropout scripts/results, dates Aug 9-11). James revealed some arguemnts that needed to be set, and the fact that the "prob_tx_droput" argument is spelled wrong (missing an o). He also included a sample script that worked, whichI have included as ageSPVL_m21_jtm2.R. This had some duplicate arguments in it (ones set twice), and some that didn't have to be set (inclding different stages of a treatment campaign). I then spent time trying to streamline it while still getting the behavior to work as expected (i.e. people having both long periods of suppression and then subsequent periods of non-suppression.) One of these attempts is ageSPVL_m21.R. Then James realized he needed to commit the function targeted_treatment2 to the master branch, which has the needed code to assign treatment randomly.  Runs 22-25 use this, and explore different scenarios. Hoewver, I did not finishing ascertaining what every parameter does and which ones are truly needed, nor did  I determine what proportion of people are actually on treatment at any point in time, what proportion go onto treatment immediately, etc.  This is all crucial for being able to understand the scnearios being modeled and relate observed outcomes to their features.  Looking at it now I see: 

Run | 21 | 22 | 23 | 24 | 25
---- | ---- | ---- | ---- | ---- | ---- 
min age	| 18 | 18 | 18 | 18 | 18
maxage | 55 | 55 | 55 | 55 | 55
mean_sqrtage_diff | 0.6 | 0.6 | 0.6 | 0.3 | 0.3
mean_sex_acts_per_day | --- | --- | --- | --- | ---
prob_sex_by_age | T | T | T | T | T
prob_sex_age_19 | 0.4 | 0.4 | 0.4 | 0.4 | 0.4 
max_age_sex | 55 | 55 | 55 | 55 | 55
relation_dur | 200 | 200 | 200 | 200 | 200 
tx_type | "random" |  |  |  | 
mean_trtmnt_delay | 1 |  |  |  | 
